 ...
 -- lock flood settings
    ...
    if text then
      if is_sudo(msg) then
        ...
        if text == 'setowner' and is_owner(msg) then
        ...
        if text and text:match('^setowner (%d+)') and is_owner(msg) then
          local user = text:match('setowner (%d+)')
          db:sadd(SUDO..'owners:'..msg.chat_id_,user)
        bot.sendMessage(msg.chat_id_, msg.id_, 1, '🌟کاربر [<b>'..user..'</b>] مالک گروه شد😊', 1, 'html')
      end
        if text == 'deowner' and is_owner(msg) then
        function prom_reply(extra, result, success)
      ...
      --------------------------master--------------------------
	   if text == 'setadmin' and is_owner(msg) then
        ...
        if text and text:match('^setadmin (%d+)') and is_owner(msg) then
          local master = text:match('setadmin (%d+)')
          db:sadd(SUDO..'masters:'..master)
        bot.sendMessage(msg.chat_id_, msg.id_, 1, '✅کاربر  [<b>'..master..'</b>] به لیست ادمین های من افزوده شد🙂', 1, 'html')
      end
        if text == 'deladmin' and is_owner(msg) then
        function prom_reply(extra, result, success)
        ...
        if text and text:match('^deladmin (%d+)') and is_owner(msg) then
 ...
 bot.sendMessage(msg.chat_id_, msg.id_, 1,'<code>✔️ ربات بروزرساني شد . . . !</code>', 1, 'html')
    ...
    local allmgs = db:get("allmsg")
     ...
     if is_owner(msg) then
        if text == 'clean bots' or text == 'پاكسازي ربات ها' and is_mod(msg) then
      ...
      for i=0 , #bots do
	   ...
	   --- charge 
	   if text and text:match('^charge (.*)') and is_sudo(msg) then
...
local dateFA = json.decode(statswelcome).FAdate
          ...
          bot.sendMessage(msg.chat_id_, msg.id_, 0 ,1 , nil, text, 1, 'md')
    ...
    if text == 'promote' or text == 'ترفیع' and is_owner(msg) then
        function prom_reply(extra, result, success)
        ...
        if text:match('^promote @(.*)') or text:match('^ترفیع @(.*)') and is_owner(msg) then
        ...
        if text and text:match('^promote @(.*)') or text:match('^ترفیع @(.*)') and is_owner(msg) then
        ...
        if text == 'demote' or text == 'عزل' and is_owner(msg) then
        function prom_reply(extra, result, success)
        ...
        if text:match('^demote @(.*)') or text:match('^عزل @(.*)') and is_owner(msg) then
        ...
        if text and text:match('^promote (%d+)') or text:match('^ترفيع (%d+)') and is_owner(msg) then
          local user = text:match('promote (%d+)')
		  local usera = text:match('ترفيع (%d+)')
          db:sadd(SUDO..'mods:'..msg.chat_id_,user)
        bot.sendMessage(msg.chat_id_, msg.id_, 1, '⭐️کاربر [<b>'..user..' or '..usera..'</b>] مدیر شد😊', 1, 'html')
      end
        if text and text:match('^demote @(.*)') or text:match('^عزل @(.*)') and is_owner(msg) then
...
-- mods
    if is_mod(msg) then
      ...
      if text and text:match('^setfloodmsg (%d+)$') and is_mod(msg) then
          db:set(SUDO..'floodmax'..msg.chat_id_,text:match('setfloodmsg (.*)'))
          bot.sendMessage(msg.chat_id_, msg.id_, 1,'🛡حساسیت فلود روی [<b>'..text:match('[Ss]etfloodmsg (.*)')..'</b>] پیام تنظیم شد🤓', 1, 'html')
        end
        if text and text:match('^setfloodtime (%d+)$') and is_mod(msg) then
          db:set(SUDO..'floodtime'..msg.chat_id_,text:match('setfloodtime (.*)'))
        ...
        if text == 'mute all' and is_mod(msg) then
        ...
        if text and text:match('^lockgp (%d+)[mhs]') or text and text:match('^[Ll]ockgp (%d+) [mhs]') and is_mod(msg) then
          local matches = text:match('^lockgp (.*)')
    ...
    if text == 'لیست بن' or text == 'banlist' and is_mod(msg) then
      ...
      if text == 'clean banlist' or text == 'پاکسازی لیست بن' and is_mod(msg) then
        db:del(SUDO..'banned'..msg.chat_id_)
          bot.sendMessage(msg.chat_id_, msg.id_, 1,'✅لیست بن پاک شد🙂', 1, 'html')
        end
        if text == 'mutelist' or text == 'لیست سکوت' and is_mod(msg) then
      ...
      if text == 'clean mutelist' or text == 'پاکسازی لیست سکوت' and is_mod(msg) then
        db:del(SUDO..'mutes'..msg.chat_id_)
          bot.sendMessage(msg.chat_id_, msg.id_, 1,'✅لیست سکوت پاک شد🙁', 1, 'html')
        end
      if text == 'kick' and tonumber(msg.reply_to_message_id_) > 0 and is_mod(msg) then
      ...
      if text and text:match('^kick (%d+)') and is_mod(msg) then
      ...
      if text and text:match('^kick @(.*)') then
        ...
        if text == 'ban' or text == 'بن' and tonumber(msg.reply_to_message_id_) > 0 and is_mod(msg) then
        function banreply(extra, result, success)
        ban(msg,msg.chat_id_,result.sender_user_id_)
          end
        bot.getMessage(msg.chat_id_, tonumber(msg.reply_to_message_id_),banreply)
        end
      if text and text:match('^ban (%d+)') or text:match('^بن (%d+)') and is_mod(msg) then
      ...
      if text == 'unban' or text == 'انبن' and tonumber(msg.reply_to_message_id_) > 0 and is_mod(msg) then
      ...
      if text and text:match('^unban (%d+)') or text:match('^انبن (%d+)') and is_mod(msg) then
      ...
      if text and text:match('^unban (%d+)') or text:match('^انبن (%d+)') and is_mod(msg) then
        ...
        if text == 'mute' or text == 'سكوت' and tonumber(msg.reply_to_message_id_) > 0 and is_mod(msg) then
      ...
      if text and text:match('^mute (%d+)') or text:match('^سكوت (%d+)') and is_mod(msg) then
      ...
      if text == 'unmute' or text == 'رفع سكوت' and tonumber(msg.reply_to_message_id_) > 0 and is_mod(msg) then
      ...
      if text and text:match('^unmute (%d+)') or text:match('^رفع سكوت (%d+)') and is_mod(msg) then
     ...
     if text and text:match('^unmute (%d+)') or text:match('^رفع سكوت (%d+)') and is_mod(msg) then
        ...
        bot.resolve_username(username,unmuteusername)
         ...
         if text == 'invite' and tonumber(msg.reply_to_message_id_) > 0 and is_mod(msg) then
      ...
      if text and text:match('^invite (%d+)') and is_mod(msg) then
        bot.addChatMembers(msg.chat_id_,{[0] = text:match('invite (%d+)')})
        end
      if text and text:match('^invite @(.*)') and is_mod then
    ...
    if text == 'modlist' or text == 'لیست مالکان' and is_mod(msg) then
      ...
      if text == 'clean modlist' or text == 'پاکسازی لیست مدیران' and is_owner(msg) then
        db:del(SUDO..'mods:'..msg.chat_id_)
          bot.sendMessage(msg.chat_id_, msg.id_, 1,'✅لیست مدیران گروه پاک شد🙁', 1, 'html')
        end
		if text == 'allowlist' or text == 'ليست مجاز' and is_mod(msg) then
      ...
      if text == 'clean allowlist' or text == 'پاکسازی لیست مجاز' and is_owner(msg) then
        db:del(SUDO..'allow:'..msg.chat_id_)
          bot.sendMessage(msg.chat_id_, msg.id_, 1,'✅لیست کاربران مجاز پاک شد🙁', 1, 'html')
        end
      if text and text:match('^filter +(.*)') or text:match('^فیلتر +(.*)') and is_mod(msg) then
      ...
      if text and text:match('^unfilter +(.*)') or text:match('^رفع فیلتر +(.*)') and is_mod(msg) then
      ...
      if text == 'clean filterlist' or text == 'پاکسازی لیست فیلتر' and is_mod(msg) then
        db:del(SUDO..'filters:'..msg.chat_id_)
          bot.sendMessage(msg.chat_id_, msg.id_, 1,'✅لیست کلمات فیلتر شده پاک شد🙂', 1, 'html')
        end
      if text == 'adminlist' or text == 'لیست ادمین ها' and is_mod(msg) then
        local function cb(extra,result,success)
      ...
      if text == 'filterlist' or text == 'لیست فیلتر' and is_mod(msg) then
    ...
    if msg_type == 'text' then
        if text then
      if text:match('^whois @(.*)') and is_mod(msg) then
        ...
        bot.sendMessage(msg.chat_id_, msg.id_, 1, '<code>شناسه-گروه</code>: {<b>'..msg.chat_id_..'</b>}', 1, 'html')
			...
			if text == 'pin' and is_mod(msg) then
        local id = msg.id_
        local msgs = {[0] = id}
